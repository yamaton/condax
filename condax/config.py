...
class BadConfigFileError(CondaxError):
    def __init__(self, message: str):
        super().__init__(10, message)


class MissingConfigFileError(CondaxError):
    def __init__(self, message: str):
        super().__init__(11, message)
...
def set_via_file(config_file: Union[str, Path]):
    ...
    try:
        ...
        raise MissingConfigFileError(f"Config file {config_file} not found")

    if not config:
        msg = f"""Config file does not contain config information: Remove or fix {config_file} and try again"""
